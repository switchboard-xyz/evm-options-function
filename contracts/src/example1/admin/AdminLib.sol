...
library AdminLib {
    ...
    struct DiamondStorage {
        ...
        // Oracle Function Id
        address functionId;
        // Switchboard Oracle Gas Fee (denominated in gas, not wei)
        uint256 switchboardGasCost;
    }
    ...
    function switchboardGasCost() internal view returns (uint256) {
        return diamondStorage().switchboardGasCost;
    }

    function setSwitchboardGasCost(uint256 _switchboardGasCost) internal {
        diamondStorage().switchboardGasCost = _switchboardGasCost;
    }
}
